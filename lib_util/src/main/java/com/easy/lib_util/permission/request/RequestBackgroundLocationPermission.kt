...
import com.easy.lib_util.permission.PermissionUtils
...
internal class RequestBackgroundLocationPermission internal constructor(permissionBuilder: PermissionBuilder)
    ...
    override fun request() {
        if (pb.shouldRequestBackgroundLocationPermission()) {
            ...
            if (PermissionUtils.isGranted(pb.activity, ACCESS_BACKGROUND_LOCATION)) {
                // ACCESS_BACKGROUND_LOCATION has already granted, we can finish this task now.
                finish()
                return
            }
            val accessFindLocationGranted = PermissionUtils.isGranted(pb.activity, Manifest.permission.ACCESS_FINE_LOCATION)
            val accessCoarseLocationGranted = PermissionUtils.isGranted(pb.activity, Manifest.permission.ACCESS_COARSE_LOCATION)
