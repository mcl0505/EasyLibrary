...
internal object DownloadAppUtils {
    ...
    fun download() {
        ...
        val apkName = if (updateInfo.config.apkSaveName.isNotEmpty()) {
            ...
            AppUtil.getApplicationName(EasyApplication.getContext())
        }
        ...
        //保存当前apk 的路径
        MmkvUtil.putValue(KEY_OF_SP_APK_PATH, downloadUpdateApkFilePath)
        ...
        downloadTask
            ...
            .setListener(object : FileDownloadLargeFileListener() {
                ...
                override fun pending(task: BaseDownloadTask, soFarBytes: Long, totalBytes: Long) {
                    LogUtil.d("----使用FileDownloader下载-------")
                    LogUtil.d("pending:soFarBytes($soFarBytes),totalBytes($totalBytes)")
                    downloadStart()
                    if(totalBytes < 0){
                        downloadTask.pause()
                    }
                ...
                override fun paused(task: BaseDownloadTask, soFarBytes: Long, totalBytes: Long) {
                    LogUtil.d("获取文件总长度失败出错，尝试HTTPURLConnection下载")
                ...
                override fun error(task: BaseDownloadTask, e: Throwable) {
                    ...
                    // downloadError(e)
                    LogUtil.d("下载出错，尝试HTTPURLConnection下载")
                    downloadUpdateApkFilePath.deleteFile()
                    "$downloadUpdateApkFilePath.temp".deleteFile()
                    downloadByHttpUrlConnection(filePath, apkName)
                }
...
//        log("soFarBytes:$soFarBytes--totalBytes:$totalBytes")
    ...
    private fun checkMd5(context: Context) {
        ...
        LogUtil.d("当前应用签名md5：$localMd5")
        LogUtil.d("下载apk签名md5：$apkMd5")
